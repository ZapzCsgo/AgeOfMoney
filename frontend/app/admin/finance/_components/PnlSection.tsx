...
const PERIOD_TOOLTIPS: Record<PnlPeriodLabel, string> = {
  today:    'Gaming margin nette sur les dernières 24 h, convertie en € au taux de retrait (1 ⚜ ≈ 0,59 €).',
  ...
  lifetime: 'Bénéfice réalisé depuis le lancement : deposits cumulés moins withdrawals cumulés moins la liability actuelle (ce que tu dois encore aux users). C\'est le cash réellement en caisse.',
};
...
function Pill({ period, loading }: { period?: PnlPeriod; loading: boolean }) {
  ...
  return (
    <div
      className="flex-1 min-w-[140px] rounded-xl p-4 relative"
      style={{
        background: '#0e0d1a',
        border: period && raw !== 0
    ...
    >
      ...
      <div className="mt-2 flex items-baseline gap-1 min-h-[28px]">
        {loading ? (
          <div className="h-6 w-24 rounded" style={{ background: 'rgba(255,255,255,0.04)' }} />
        ) : (
          <>
